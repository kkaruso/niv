...
class BuildDiagram:
    ...
    def __init__(self, load_path, save_path, detail_level):
        ...
        self.nodes_text = self.fill_dictionary("icons", "text", self.yaml_defaults.get('icons').get(
            'text') or "node")

        # Get ip of each node
        self.nodes_ip = self.fill_dictionary("icons", "ip", self.yaml_defaults.get('icons').get(
            'ip') or "")

        # Get port of each node
        self.nodes_port = self.fill_dictionary("icons", "port", self.yaml_defaults.get('icons').get(
            'port') or "")

        # Get the URL of each node, clear empty URLs
        self.nodes_url = self.fill_dictionary("icons", "url", self.yaml_defaults.get('icons').get(
            'url') or "")
        ...
        if self.graph_layout == "neato":
            ...
            self.nodes_x = self.fill_dictionary("icons", "x", self.yaml_defaults.get('icons').get(
                'x') or 0)

            # Get Y coordinate of each node
            self.nodes_y = self.fill_dictionary("icons", "y", self.yaml_defaults.get('icons').get(
                'y') or 0)
        ...
        _dict = {"Title": self.set_variables("title", "text", self.yaml_defaults.get('title').get(
            'text') or "Diagram"),
                 ...
                 "Author": self.set_variables("title", "author", self.yaml_defaults.get('title').get(
                     'author') or ""),
                 "Date": self.set_variables("title", "date", self.yaml_defaults.get('title').get(
                     'date') or datetime.today().strftime('%d.%m.%Y')),
